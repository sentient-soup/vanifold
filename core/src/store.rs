...
use crate::model::{now_ms, Device, Entity};
...
pub enum StoreMsg {
    ...
    Point { entity_id: String, ts: u64, value: f64 },
}
...
pub fn load(conn: &Connection) -> rusqlite::Result<Loaded> {
    ...
    Ok(Loaded { devices, entities, quarantine })
}
...
fn flush(conn: &mut Connection, points: &mut Vec<(String, u64, f64)>) {
    ...
    let result = (|| -> rusqlite::Result<()> {
        ...
        {
            let mut stmt = tx.prepare_cached("INSERT INTO history_raw(entity_id, ts, value) VALUES(?1, ?2, ?3)")?;
...
pub fn history(path: &Path, entity_id: &str, from: u64, to: u64) -> rusqlite::Result<Vec<HistoryPoint>> {
    let conn = Connection::open(path)?;
    let mut out = Vec::new();
    for (table, is_raw) in [("history_1h", false), ("history_1m", false), ("history_raw", true)] {
        let sql = if is_raw {
            format!("SELECT ts, value, value, value FROM {table} WHERE entity_id = ?1 AND ts >= ?2 AND ts <= ?3 ORDER BY ts")
        } else {
            format!("SELECT ts, vmin, vmax, vavg FROM {table} WHERE entity_id = ?1 AND ts >= ?2 AND ts <= ?3 ORDER BY ts")
        };
...
mod tests {
    ...
    fn downsample_tiers() {
        ...
        for (ts, v) in [(old, 10.0), (old + 1000, 20.0), (now - 1000, 5.0)] {
            conn.execute("INSERT INTO history_raw(entity_id, ts, value) VALUES('e1', ?1, ?2)", (ts as i64, v))
                .unwrap();
        }
        ...
        let raw_left: i64 = conn.query_row("SELECT COUNT(*) FROM history_raw", [], |r| r.get(0)).unwrap();
        ...
        let (vavg_1h, n_1h): (f64, i64) = conn
            .query_row("SELECT vavg, n FROM history_1h", [], |r| Ok((r.get(0)?, r.get(1)?)))
            .unwrap();
        assert_eq!((vavg_1h, n_1h), (15.0, 2));
        let m_left: i64 = conn.query_row("SELECT COUNT(*) FROM history_1m", [], |r| r.get(0)).unwrap();
        assert_eq!(m_left, 0);
    }
